...
/**
  * Specification for trying to add a new book with no author to the catalog
  * using the service
  * @author Kyle Cranmer
  * @since 0.2
  */
class TryingToAddBookWithNoAuthorSpec
    extends PropSpec
    with Matchers
  ...
  property("indicates the book was not added to the catalog") {
    forAll(catalogGenerator, repositoryGenerator, noAuthorGenerator) {
      (
        catalog: BookCatalog,
