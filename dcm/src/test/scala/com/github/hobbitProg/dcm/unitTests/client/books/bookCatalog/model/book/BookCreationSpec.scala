...
class BookCreationSpec
  ...
  "Given book information with no title" >> {
    "no book is created" >> {
      Prop.forAllNoShrink(noTitleDataGenerator) {
        (bookData: BookDataType) => {
          bookData match {
            case (title, author, isbn, description, cover, categories) =>
              val newBook =
  ...
  "Given book information with no author" >> {
    "no book is created" >> {
      Prop.forAllNoShrink(noAuthorDataGenerator) {
        (bookData: BookDataType) => {
          bookData match {
            case (title, author, isbn, description, cover, categories) =>
            val newBook =
